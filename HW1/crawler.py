...
class Crawler:
    ...
    def __init__(self, base_url, root_url, urls_limit, depth_limit, delay, keyword=None):
        self._urls = list()
    ...
    def crawl_pages(self, url, depth, index):
        '''
        Pages are crawled here and checks are done for each item before crawling to 
        keep note of the limit or depth provided
        '''
    ...
    def write_page(self, request_data, depth, index):
        file_name = 'data/depth_{}_index_{}.html'.format(depth, index)
    ...
    def start(self):
        ...
        if not os.path.exists('./data'):
            ...
            os.mkdir('./data')
        self.crawl_pages(self._root_url, self._depth, 0)
...
if __name__ == "__main__":
    ...
    if keyword is None:
        # Non focussed mode
        crawler = Crawler(URL_BASE, root_url, urls_limit=url_limit, depth_limit=depth_limit, delay=delay)
        crawler.start()
    else:
        # Focused crawler
        crawler = Crawler(URL_BASE, root_url, urls_limit=url_limit, depth_limit=depth_limit, delay=delay, keyword=keyword)
        crawler.start()
